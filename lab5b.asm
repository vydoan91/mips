# Lab 5b: Take your matrix multiply program from Lab#4 and unroll the inner loop 
# once (two iterations of the unrolled loop)
...
matrix: # need address of matrix1 or 2 in s2 to execute this procedure
	...
	exitOutputLoop:
		jr $ra
multiply: 
# Unrolling inner loop with 2 interations in C language
...
#		 c[i][j] = c[i][j] + a[i][k+1]*b[k+1][j]
	...
	L2: 
		...
		lw $t4, 0($t2) 		# $t4 = c[i][j]
	L3: # two iterations of the unrolled loop
		addi $s3,$s2,1		# s3=1
